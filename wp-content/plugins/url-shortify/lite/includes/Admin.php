...
class Admin {
	...
	public function add_admin_menu() {
		...
		if ( count( $permissions ) > 0 ) {
			...
			add_menu_page( __( 'URL Shortify', 'url-shortify' ), __( 'URL Shortify', 'url-shortify' ), 'edit_posts', 'url_shortify', array( $this, 'render_dashboard' ), 'dashicons-admin-links', 30 );

			if ( in_array( 'manage_links', $permissions ) || in_array( 'create_links', $permissions ) ) {

				// Dashboard
				add_submenu_page( 'url_shortify', __( 'Dashboard', 'url-shortify' ), __( 'Dashboard', 'url-shortify' ), 'edit_posts', 'url_shortify', array( $this, 'render_dashboard' ) );

				// Links
				$hook = add_submenu_page( 'url_shortify', __( 'Links', 'url-shortify' ), __( 'Links', 'url-shortify' ), 'edit_posts', 'us_links', array( $this, 'render_links_page' ) );
			}

			if ( in_array( 'manage_groups', $permissions ) ) {
				$hook = add_submenu_page( 'url_shortify', __( 'Groups', 'url-shortify' ), __( 'Groups', 'url-shortify' ), 'edit_posts', 'us_groups', array( $this, 'render_groups_page' ) );
			}

			if ( US()->is_pro() && in_array( 'manage_custom_domains', $permissions ) ) {
				$hook = add_submenu_page( 'url_shortify', __( 'Domains', 'url-shortify' ), __( 'Domains', 'url-shortify' ), 'edit_posts', 'us_domains', array( $this, 'render_domains_page' ) );
			}

			if ( in_array( 'manage_settings', $permissions ) ) {

				if ( Helper::is_mts_short_links_table_exists() || Helper::is_pretty_links_table_exists() ) {
					$hook = add_submenu_page( 'url_shortify', __( 'Tools', 'url-shortify' ), __( 'Tools', 'url-shortify' ), 'edit_posts', 'us_tools', array( $this, 'render_tools_page' ) );
				}
	...
	public function update_admin_footer_text( $footer_text ) {
		...
		if ( Helper::is_plugin_admin_screen() ) {
			...
			$footer_text = sprintf( __( '<span id="footer-thankyou">Thank you for creating with <a href="%1$s" target="_blank">WordPress</a> | URL Shortify <b>%2$s</b>. Developed by team <a href="%3$s" target="_blank">KaizenCoders</a></span>', 'url-shortify' ), $wordpress_url, KC_US_PLUGIN_VERSION, $website_url );
		}
	...
	public function show_custom_notices() {
		...
		if ( ! US()->is_pro() && Helper::is_plugin_admin_screen() ) {
			include_once KC_US_ADMIN_TEMPLATES_DIR . '/promo/welcome.php';
		}
