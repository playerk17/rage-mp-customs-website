...
class Install {
	...
	const GEOLITE2_DB = 'http://kaizencoders.com/downloads/geoip/GeoLite2-Country.mmdb.tar.gz';
	...
	private static $db_updates = array(
		...
		'1.3.9' => array(
			'kc_us_update_139_update_geoip_db'
		),
	...
	public static function check_version() {
		...
		self::maybe_update_geo_database();
	}
	...
	/**
	 * Update Geo database every week
	 *
	 * @param bool $force
	 *
	 * @since 1.0.3
	 */
	public static function maybe_update_geo_database( $force = false ) {

		$option       = 'geo_database_last_updated_on';
		$last_updated = Option::get( $option, 0 );

		if ( ! file_exists( KC_US_GEO_COUNTRY_DB_PATH ) ) {
			$force = true;
		}

		if ( $force || ( $last_updated <= time() - ( 7 * 24 * 60 * 60 ) ) ) {

			require_once ABSPATH . 'wp-admin/includes/file.php';

			$database             = 'GeoLite2-Country.mmdb';
			$target_database_path = KC_US_GEO_COUNTRY_DB_PATH;
