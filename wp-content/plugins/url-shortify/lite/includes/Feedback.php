			...
			if ( $enable_review_notice ) {
				...
				if ( $can_ask_user_for_review ) {
					...
					if ( $nonce_verified ) {
						...
						if ( ! empty( $action ) && $this->is_valid_action( $action ) ) {
							...
							if ( $action === $review_done_option ) {
								...
								$url = ! empty( $_GET['review_url'] ) ? eesc_url( $_GET['review_url'] ) : '';
					...
					if ( ! $got_feedback ) {
						...
						echo sprintf( '<li><a href="%s" class="text-white px-4 py-2 ml-6 mr-2 align-middle cursor-pointer kc-us-primary-button" target="_blank" data-rated="' . esc_attr__( "Thank You :) ",
